 ...
 *
 * Note: Using CommonJS for Jest compatibility with ES modules
 */
...
describe('JWT Token Security Tests', () => {
  ...
  describe('Token Validation', () => {
    ...
    it('should set req.user and req.userId when token is valid', async () => {
      ...
      // If middleware sets req.userId correctly, we should get correct user
      expect(response.body.data.id).toBe(testUser.id);
    });
  });
  ...
  describe('Token Expiration', () => {
    ...
    it('should accept token that expires in 1 hour', async () => {
      const token = jwt.sign(
        { userId: testUser.id, email: testUser.email },
        process.env.JWT_SECRET,
        { expiresIn: '1h' }
  ...
  describe('Security Best Practices', () => {
    ...
    it('should not accept token with tampered payload', async () => {
      ...
      // Tamper with token (change userId)
      const [header, payload, signature] = validToken.split('.');
      const decodedPayload = Buffer.from(payload, 'base64').toString();
      const tamperedPayload = decodedPayload.replace(testUser.id, 'tampered-id');
